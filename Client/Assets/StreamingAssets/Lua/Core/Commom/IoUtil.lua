...
local path = cc.FileUtils:getInstanceance():fullPathForFilename("gamedata.txt")
...
function io.exists(path)
    if VersionConfig.IS_SERVER then
        --VersionConfig.LUA_ROOT_PATH 需要带 /
        --LuaScripts/Battle/...
        local file = io.open(VersionConfig.LUA_ROOT_PATH.. path, "r")
        if file then
            io.close(file)
            return true
        end
        Logger.log( VersionConfig.LUA_ROOT_PATH.. path )
        return false
    end
    return CS.wc.framework.FileHelper.Instance:FileExist(path);
end


function io.fileFullPath(path)
    local fulL_path = CS.wc.framework.FileHelper.Instance:GetExistPath(path)
    return fulL_path
end
...
function io.readfile(path)
    if VersionConfig.IS_SERVER then
        ...
        local content = CS.wc.framework.FileHelper.Instance:ReadAllText(path)
        return content
    end
end
...
--写入战斗log
function io.writelog(path, content, mode)
    local log_path = VersionConfig.LOG_PATH..path
    if VersionConfig.IS_SERVER then
        mode = mode or "w+b"
        ...
        if file then
            ...
            io.close(file)
            Logger.log("server log file export "..log_path.." success")
            return true
        else
            return false
        end
    else
        CS.wc.framework.FileHelper.Instance:WriteText(content)
        Logger.log("client log file "..log_path.." export success")
    end
end
...
function io.mkdir(path)
    CS.wc.framework.FileHelper.Instance:CreateFolderByFile( CS.wc.framework.FileHelper.Instance.readAndWritePath .. path)
end

function io.deletedir(path)
    CS.wc.framework.FileHelper.Instance:DeleteDir(path)
end